...
import { LogOut } from 'lucide-react'; // Importer l'icône de déconnexion
...
export default function Home() {
  ...
  useEffect(() => {
    // Seulement initialiser si connecté
    if (isConnected) {
      const initOBS = async () => {
        try {
          ...
          setIsConnected(false); // Revenir à l'écran de connexion en cas d'erreur
        }
      };

      initOBS();
    }
  }, [isConnected]); // Exécuter seulement quand isConnected change
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-4xl mx-auto py-4 px-8 flex flex-col md:flex-row md:justify-between items-center text-center gap-4">
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-8 space-y-6 sm:space-y-8">
        {!isConnected ? (
          <ConnectionForm onConnected={() => setIsConnected(true)} />
        ) : (
          <>
            {/* Status Cards */}
